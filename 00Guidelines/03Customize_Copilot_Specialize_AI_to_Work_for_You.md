# Customize Copilot: Specialize AI to Work for You

## When To Use AI in SDLC
...
```md
- .github/: Config files for AI Agent behavior in VS Code.
- .vscode/: Config files for VS Code.
- 00Guidelines/: Guides for setup, notebooks, and Copilot customization.
...
- 04Deployment/: Deployment-related assets.
- Archive/: Archived materials.
```

## Config Files

The `.github/` folder contains Copilot configuration for this workspace.

Current items are: `copilot-instructions.md`, `prompts/`, `skills/`, and `instructions/`.

**Use a `.github/copilot-instructions.md` file**

[This file](../.github/copilot-instructions.md) is automatically applied to chat requests in the workspace.

Use `/init` to generate or update this file.
...
Create file-scoped instructions in `.github/instructions/*.instructions.md`.

They are applied dynamically based on the files or tasks being handled.

[sql-script-terminator.instructions.md](../.github/instructions/sql-script-terminator.instructions.md) is for files ending in `.sql`, `.pkb`, or `.pkg`.

Type `/create-instructions`, then add a description of the instructions you want to generate.
...
Prompt files are stored under `.github/prompts/`. Current prompt files are:
...
Agent skills enable specialized capabilities and workflows that can include scripts, examples, and other resources.

Agent skills are stored in `.github/skills/[skill-name]/SKILL.md` and are loaded when relevant for specialized tasks.

Current active skill folders include:

| Skill | Description | Example |
|---|---|---|
| `/db-audit` | Use Oracle MCP to inspect `DBTOOLS$MCP_LOG` with read-only SQL for recent log entries, failed call details, endpoint activity, and slow operations only. | `/db-audit Show the latest MCP log rows` |
| `/find-skills` | Help users discover and install agent skills from the skills ecosystem when they ask for a capability, workflow, or specialized task support. | `/find-skills Find a skill for PR review` |
...
Hooks enable you to automate workflows, enforce security policies, validate operations, and integrate with external tools.

**Microsoft Extension**

`VS Code Speech` brings speech-to-text and other voice capabilities to VS Code.

![VS Code Speech extension screenshot](image-7.png)

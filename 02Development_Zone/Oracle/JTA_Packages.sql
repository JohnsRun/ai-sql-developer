...
-- Prerequisite: run JTA_Create_Database.sql in this same schema before this script.

----------------------------------------------------------------------
-- |Version |Author |Date      |Description
-- |v003    |Admin  |2026-09-08|Clarify database script dependency
----------------------------------------------------------------------
--v003s
...
*/
CREATE OR REPLACE PACKAGE pkg_jta_error
IS
...
-- package body for pkg_jta_error
CREATE OR REPLACE PACKAGE BODY pkg_jta_error
IS
...
CREATE OR REPLACE PACKAGE pkg_jta 
IS
    -- some constants
    ...
    PROCEDURE sp_get_recommended_price_for (
    ...
    PROCEDURE sp_evaluate_po_order_line (
...
-- JTA Package Body
CREATE OR REPLACE PACKAGE BODY pkg_jta IS

    PROCEDURE sp_get_date (
    ...
    BEGIN
       ...
       -- note that the procedure essentially do nothing if there are no work data for this week 
        ...
        FOR current_staff IN c_staff_worked LOOP
            ...
            sp_get_hours(current_staff.staff_id, v_start_date, v_end_date, basic, overtime, doubletime);
            -- get staff pay rate
            ...
            INSERT INTO payroll ( 
                ...
                net_pay)
            VALUES (
                seq_payroll_id.NEXTVAL, current_staff.staff_id, v_start_date, v_end_date,
        ...
        ELSE 
            ...
            pkg_jta_error.sp_throw(-20201, 'There was no work hours recorded for this week: '
                || to_char(v_start_date, 'yyyy-Mon-dd, HH24:MI:SS') || ' to '
                || to_char(v_end_date, 'yyyy-Mon-dd, HH24:MI:SS'));
        END IF;
        
    EXCEPTION
        WHEN pkg_jta_error.invalid_input THEN
            pkg_jta_error.sp_show_in_console(SQLCODE, SQLERRM);
            ROLLBACK;
        WHEN OTHERS THEN
            -- all OTHER exceptions will be logged into error table...
            pkg_jta_error.sp_log_error(SQLCODE, SQLERRM);  
            ROLLBACK;
    ...
    */
    ...
    BEGIN
        ...
        IF p_quantity = 0 THEN
            pkg_jta_error.sp_throw(-20201, 'cannot update a zero amount to inventory');
        END IF;
        ...
        EXCEPTION
            WHEN no_data_found THEN
                pkg_jta_error.sp_throw(-20201, 'non existing product being updated to inventory');
            WHEN OTHERS THEN
        ...
        IF p_quantity > 0 THEN
            ...
            IF p_new_cost <= 0 OR p_new_cost IS NULL THEN
                pkg_jta_error.sp_throw(-20201, 'cannot update inventory with non positive cost per unit');
            ELSE
                v_cost := p_new_cost;            
            END IF;
        ...
        ELSE
            ...
            IF v_new_total < 0 THEN
                pkg_jta_error.sp_throw(-20201, 'cannot remove more items than already exists in inventory');
            END IF;
        ...
        VALUES (
            seq_transaction_id.NEXTVAL, p_product_id, v_direction, sysdate, p_quantity,
    ...
    */
    ...
    BEGIN
        ...
        FOR supplier IN suppliers LOOP
            ...
            v_current_po := seq_po_id.NEXTVAL;
            ...
            FOR stock IN urgent(supplier.supplier_id, supplier.location_id) LOOP
                ...
                VALUES(seq_po_line_id.NEXTVAL, v_current_po, stock.product_id, stock.reorder_level, NULL );
            END LOOP;
    ...
    */
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ...
            RETURN NULL;
    END fn_get_last_cashier_payout;
    ...
    */
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN no_data_found THEN
                pkg_jta_error.sp_throw(-20201, 'location does not exist');
        END;
        ...
        ELSIF p_type = 'non-cash' THEN
            ...
            AND cs.location_id = p_location_id;
        ELSE
            pkg_jta_error.sp_throw(-20201, 'invalid type, valid types: cash, non-cash');
        END IF;
        ...
        FOR new_rate IN changes LOOP
            ...
            BEGIN
                ...
                IF SQL%rowcount = 0 THEN
                    pkg_jta_error.sp_throw(-20201, 'attempt to update tax to same rate');
                END IF;
    ...
    */
    ...
    BEGIN
        ...
        BEGIN
        sp_get_date(v_current_time);
        dbms_output.put_line('Current local time: ' || TO_CHAR(v_current_time, 'yyyy-Mon-dd HH24:MI:SS'));
       END;
    ...
    */
    ...
    BEGIN
        ...
        EXCEPTION
            ...
            pkg_jta_error.sp_throw(-20202, 'product does not exist or has not been added to inventory');
        END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            p_old_price := NULL;
            p_new_price := NULL;
    END sp_get_recommended_price_for;
    ...
    */
    ...
    EXCEPTION
        WHEN no_data_found THEN
            ...
            pkg_jta_error.sp_log_error(-20202, 'purchase not exist or inventory has never been added for this item');
            p_not_enough_to_restock := TRUE;
            p_not_at_min_level := TRUE;
        WHEN OTHERS THEN
            -- all OTHER exceptions will be logged into error table...
    ...
    */
    ...
    BEGIN
        ...
        sp_get_hours(v_staff_id, v_start_date, v_end_date, v_basic, v_overtime, v_doubletime);
    ...
    EXCEPTION
        WHEN no_data_found THEN
            ...
            pkg_jta_error.sp_log_error(-20202, 'price look up for item that does not exist');
            p_product_id := NULL;
            p_product_name := NULL;
            p_price_rate := NULL;
            p_tax_code := NULL;
        ...
        FOR item IN items_in_bill LOOP
            ...
            IF item.barcode IS NOT NULL THEN
                ...
                BEGIN
                    ...
                    IF SQL%rowcount = 0 THEN
                        pkg_jta_error.sp_throw(-20202, 'product not in inventory');
                    END IF;
                    ...
                    VALUES (seq_sold_products.NEXTVAL, item.product_id, item.quantity);
                EXCEPTION
                    WHEN OTHERS THEN
    ...
    */
    ...
    BEGIN
        ...
        FOR indx IN v_sum.FIRST..v_sum.LAST LOOP
            ...
            sp_update_inventory (indx, (v_sum(indx) * -1), NULL);
        END LOOP;
    ...
    */
    ...
    BEGIN 
        ...
        EXCEPTION
            WHEN no_data_found THEN
                pkg_jta_error.sp_throw(-20201, 'failed to update bill items because bill status not unpaid');
        END;
        
        -- find price and tax info using lookup_barcode procedure
        pkg_jta.sp_lookup_barcode (p_barcode, v_product_id, v_name, v_price, v_tax_code, v_tax_rate);
        ...
        ELSE
            ...
            VALUES (
                seq_bill_line_id.NEXTVAL, p_bill_id, v_product_id, 1,
    ...
    */
    ...
    BEGIN 
        ...
        IF p_type NOT IN ('cash', 'cheque', 'creditcard', 'linx') THEN
            pkg_jta_error.sp_throw(-20201, 'invalid payment type, valid types: cash, cheque, creditcard, linx');
        END IF;
        ...
        IF v_status = 'paid' THEN
            ...
            pkg_jta_error.sp_throw(-20201, 'attempt to pay on bill that has already received full payment');
        end if;
        
        -- check if amount is valid, change status depending on if full amount is paid
        IF p_amount <= 0 THEN
            -- throw (raise) invalid input error
            pkg_jta_error.sp_throw(-20201, 'invalid money amount, negative number');
        end if;
        ...
        IF v_do_update_inventory THEN
            pkg_jta.sp_update_from_bill(p_bill_id);
        END IF;
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN no_data_found THEN
                ...
                pkg_jta_error.sp_throw(-20201, 'invalid location or product id when finding quantity sold');
        END;
    ...
    */
    ...
    BEGIN
        ...
        IF p_in_stock > p_value_counted THEN
            ...
            VALUES (seq_m_item_id.nextval, p_product_id, sysdate, v_difference);
            ...
            pkg_jta.sp_update_inventory(p_product_id, v_difference, NULL);
...
BEGIN
    ...
    VALUES (seq_event.nextval, USER, SYSDATE, ora_sysevent, ora_client_ip_address);
...
BEGIN
    -- log event
    INSERT INTO jta_events (event_id, user_name, date_time, event, ip_address)
    VALUES (seq_event.nextval, USER, SYSDATE, ora_sysevent, NULL);

-- Earl Tilluck
-- These anonymous blocks can be used to run and test each procedure/function in the pkg_jta package.
-- Note: These are *quick and dirty* blocks of code for testing only, 

----------------------------------------------------------------------
-- |Version |Author |Date      |Description
-- |v002    |Admin  |2026-09-08|Apply database object naming conventions
----------------------------------------------------------------------
--v002s
...
*/
    ...
    BEGIN
        ...
        pkg_jta.sp_update_inventory(1, 200, 175);
        -- remove 100 of the same product 
        pkg_jta.sp_update_inventory(1, -100, NULL);
    END;
    /
    ...
    BEGIN
        ...
        pkg_jta.sp_update_inventory(9, 200, 175);
        -- bad input, zero products
        pkg_jta.sp_update_inventory(1, 0, 175);
        -- bad input, null price when adding
        pkg_jta.sp_update_inventory(1, 200, NULL);
    END;
    /
...
*/
    ...
    BEGIN
        -- only staff no 11 has cashier data in db
            dbms_output.put_line('payout for id: ' || v_id || ' = ' || pkg_jta.fn_get_last_cashier_payout(v_id));
    END;
    /
...
*/
    BEGIN
        dbms_output.put_line(
            ...
            || pkg_jta.fn_get_money_inflow(10, TO_DATE('2015-01-01', 'yyyy-mm-dd'), 
...
*/
    DECLARE
        v_tab pkg_jta.price_changes_table;
    BEGIN
        -- call procedure 09, this returns a collection type
        v_tab := pkg_jta.fn_get_price_changes(2, TO_DATE('01-JAN-14', 'dd-MON-yy'), TO_DATE('31-DEC-14', 'dd-MON-yy'));
...
*/
    DECLARE
        ...
        v_net_gain NUMBER;
    BEGIN
        pkg_jta.sp_get_profits_for(v_start_date, v_end_date, v_goods_sold,
...
*/
    DECLARE
        ...
        v_new_price NUMBER;
    BEGIN
        pkg_jta.sp_get_recommended_price_for (v_product_id, v_avg_cost, v_old_price, v_new_price);
...
*/
    ...
    BEGIN
        ...
        pkg_jta.sp_evaluate_po_order_line (v_line_id, v_restock, v_level);
...
*/
    DECLARE
        ...
        v_rate NUMBER;
    BEGIN
        pkg_jta.sp_lookup_barcode('212340012062', v_id, v_name, v_price, v_code, v_rate);
        dbms_output.put_line(v_id || ', ' || v_name || ' - ' || v_price || ', tax info: ' || v_code || ' - ' || v_rate); 
        pkg_jta.sp_lookup_barcode('1234567879111', v_id, v_name, v_price, v_code, v_rate);
        dbms_output.put_line(v_id || ', ' || v_name || ' - ' || v_price || ', tax info: ' || v_code || ' - ' || v_rate); 
    END;
    /
...
*/
    ...
    DECLARE
        v_bill_id NUMBER := seq_bill_id.NEXTVAL;
        v_change NUMBER;
    BEGIN
        ...
        pkg_jta.sp_add_item_to_bill(v_bill_id, '1234567879111');
        pkg_jta.sp_add_item_to_bill(v_bill_id, '1234567879111');
        -- add soap to bill
        pkg_jta.sp_add_item_to_bill(v_bill_id, '1234567879222');
        
        -- receive payment for bill
        v_change := pkg_jta.fn_receive_payment(v_bill_id, 'cash', 500);
...
*/
    ...
    BEGIN
        ...
        v_change := pkg_jta.fn_receive_payment(1, 'cash', 10);
        IF v_change IS NOT NULL THEN
            dbms_output.put_line('your change is: ' || to_char(v_change, '$999.99' ));    
        END IF;
    END;
    ...
    BEGIN
        ...
        v_change := pkg_jta.fn_receive_payment(1, 'cash', 500);
        IF v_change IS NOT NULL THEN
            dbms_output.put_line('your change is: ' || to_char(v_change, '$999.99' ));    
        END IF;
    END;
    ...
    BEGIN
        ...
        v_change := pkg_jta.fn_receive_payment(2, 'linx', 800.00);
        IF v_change IS NOT NULL THEN
            dbms_output.put_line('your change is: ' || to_char(v_change, '$999.99' ));    
        END IF;
        
        dbms_output.put_line('pay bill 2 again, should fail because already paid');
        v_change := pkg_jta.fn_receive_payment(2, 'linx', 699.78);
        IF v_change IS NOT NULL THEN
            dbms_output.put_line('your change is: ' || to_char(v_change, '$999.99' ));    
        END IF;
...
*/
    ...
    BEGIN
        ...
        dbms_output.put_line('VAT due for 2015 = ' ||
            pkg_jta.fn_get_tax_payment_due( 1, TO_DATE('2015-09-09','yyyy-mm-dd') )
        );    
    END;
    /
...
*/
    ...
    BEGIN
        dbms_output.put_line('products sold = ' ||
            pkg_jta.fn_get_quantity_sold(v_product, v_location, 
...
*/
    ...
    BEGIN
        ...
        pkg_jta.sp_stock_check(v_product, v_location, v_counted, v_in_stock);
...
*/
    DECLARE
        ...
        v_av BOOLEAN;
    BEGIN
        pkg_jta.sp_sunday_check(v_id, TO_DATE('2015-03-01', 'yyyy-mm-dd'), v_sun, v_av);
...
*/
    ...
    BEGIN
        ...
        pkg_jta.sp_payout (indx, v_begin, v_end, v_gp, v_np, v_hlt, v_nat, v_ded);
        -- print results 
        dbms_output.put_line('Staff member: ' || pkg_jta.fn_get_name(indx));
